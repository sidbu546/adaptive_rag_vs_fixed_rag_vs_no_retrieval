...
| Finding | Detail |
|---------|--------|
| Lowest Hallucination | **Adaptive RAG** (0.1958) |
| Highest Groundedness | **Adaptive RAG** (0.6762) |
| Fastest Response | **No Retrieval LLM** (20.49s) |
...
**Takeaway:** Adaptive has lower hallucination (Diff=-0.1102, p=0.0001***) and higher groundedness (Diff=0.0502, p=0.0875), while also improving answer relevance (Diff=1.2344, p=0.0000***). Conceptually, this indicates adaptive retrieval policy is not only changing retrieval quantity but improving evidence-to-answer alignment: the model is more often using retrieved context to produce faithful answers instead of over-committing to weak context. The operational trade-off is orchestration complexity and some latency/memory sensitivity, but the quality signal is stronger on factual reliability metrics.
...
**Takeaway:** Adaptive strongly improves factual reliability vs no-retrieval: hallucination drops by 0.8042 and groundedness rises by 0.6762 (both 0.0000***/0.0000***). No-retrieval remains faster on response time by 7.20s (p=0.0000***). Conceptually, this shows the classic speed-vs-trustworthiness frontier: removing retrieval can reduce pipeline overhead, but it shifts the system toward unguided generation where responses may stay fluent yet become weakly grounded. For production factual QA, adaptive retrieval acts as a reliability control layer rather than a mere performance optimization.
...
**Takeaway:** Fixed also materially outperforms no-retrieval on hallucination and groundedness (Diff=-0.6940 / 0.6261, p=0.0000***/0.0000***) and gains answer relevance by -1.2422 (p=0.0000***). Conceptually, even a non-adaptive retrieval baseline is a substantial upgrade over no-retrieval for factual tasks because it constrains answer generation to external evidence. The remaining gap between fixed and adaptive can then be interpreted as policy flexibility: fixed retrieval improves grounding, while adaptive retrieval further improves how context is selected and used under varying query difficulty.
