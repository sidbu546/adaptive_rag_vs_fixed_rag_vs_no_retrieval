...
from rag_core import rag_advanced, make_classifier
...
def evaluate_rag(
    ...
    # Build classifier once per evaluation run (not once per query)
    classifier = make_classifier(mode=classifier_mode, llm_manager=rag_llm)
    print(f"[evaluate_rag] classifier_mode={classifier_mode}  "
          f"type={type(classifier).__name__}")
