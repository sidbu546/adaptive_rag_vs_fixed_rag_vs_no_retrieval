...
PDF_PATH = str(DATA_DIR / "Medium_corpus.pdf")
OUTPUT_CSV_PATH = str(OUTPUTS_DIR / "F_Medium_fixed_rag_results_llama_4bit.csv")

PROMPTS = [
    "What are the three broad elements introduced early in the book: applications, data, and problems?",
    "Which chapter covers optimization methods?",
    "Which chapter introduces kernels and function spaces?",
    "Which chapter discusses support vector classification?",
    "Why is machine learning preferred over hand-crafted rules in web search ranking?",
    "How is collaborative filtering similar to webpage ranking despite lacking an explicit query?",
    "Why can spam filtering and cancer diagnosis both be treated as classification problems?",
    "Why does data normalization matter when vector features use different scales?",
    "Explain how many real-world problems can be reduced to a small number of learning prototypes.",
    "How does the book connect probability theory with machine learning algorithms?",
    "Why is optimization central to many machine learning methods?",
    "How do supervised learning tasks differ from density estimation tasks in objectives?",
    "Compare classification and regression tasks.",
    "Compare generative and discriminative modeling approaches.",
    "Compare nearest neighbor methods and parametric classifiers.",
    "Compare kernel methods and linear models.",
    "What does it mean for a model to learn?",
    "Why does more data sometimes help but not always?",
    "What makes one algorithm better than another?",
    "How can a model generalize to data it has never seen before?"
]
...
# Start conservative.
GPU_MAX_MEMORY = "44GiB"   # try 34GiB if OOM persists
...
class GPUMonitor:
    def __init__(self, interval_sec: float = 0.2):
        ...
        if self.enabled:
            ...
            # Map to the actual physical GPU index
            visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES", "0")
            physical_gpu_index = int(visible_devices.split(",")[0])
