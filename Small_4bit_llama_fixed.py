...
# Safe for Llama 3.3 70B Instruct 4-bit on NVIDIA L40S / A6000 class GPU
# ============================================================

# IMPORTANT:
# 1) Restart kernel before running.
# 2) Set environment variables BEFORE importing torch/transformers.
# 3) Make sure the local Llama folder actually contains model files.
# 4) If tokenizer loading fails, install:
#       %pip install -q sentencepiece tiktoken bitsandbytes accelerate
# ============================================================
...
OUTPUT_CSV_PATH = str(OUTPUTS_DIR / "fixed_rag_results_llama33_70b_4bit.csv")
...
GPU_MAX_MEMORY = "40GiB"   # L40S has 48GB; reduce if needed
...
def assert_local_llama_files_exist(local_model_path: str):
    ...
    if not files:
        raise FileNotFoundError(
            ...
            f"You must download the model files into this directory first."
        )
    ...
    if not has_weights:
        raise FileNotFoundError(
            f"No model weights (.safetensors or .bin) found in {local_model_path}"
        )
