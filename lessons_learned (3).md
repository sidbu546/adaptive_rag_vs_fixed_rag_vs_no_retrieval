...
### 1.7 Citation-link rendering and UI consistency during migration

During the migration to Chainlit, citation rendering and UI consistency issues appeared in the frontend layer (branding, landing content placement, follow-up button behaviour, and footer overlap with the composer). The current implementation resolves these through Chainlit UI customization assets (`public/custom.css` and `public/custom.js`) plus explicit action callbacks in `chainlit_ui.py`.
...
9. **Be honest about dead code.** The staging prompt and parts of the Ollama pathway remain in the repository even though the primary app path is now `chainlit_ui.py` with OpenAI default. Future maintainers will assume all paths are active. Either delete dead code or annotate it clearly as unused.
...
### 2.1 UI Evolution Completed in This Iteration

- **Shift from Q&A page to chat UX.** The interface moved from a mostly single-turn question/answer format to a true chat-style experience using Chainlit.
- **Conversation flow is now natural.** Users can continue with follow-up questions in the same thread, and the UI supports scrolling through earlier turns.
- **Interactive entry points.** Suggested questions are shown as clickable chat starters/follow-ups for faster onboarding.
- **PDF-aware interaction.** Users can upload PDFs and ask questions grounded in uploaded documents during the session.
- **Branding and safety UX.** The landing experience includes app branding and disclaimer messaging, with a persistent footer disclaimer integrated into the UI.
...
- **Persistent memory across sessions.** The current Chainlit app keeps multi-turn context within a live session, but does not yet persist full user history across login sessions. Add authenticated, durable conversation history so users can return to prior threads.
...
- **Enhanced document handling.** Build on the existing PDF upload flow with stronger OCR coverage, better document previews, and per-document citation mapping back to uploaded files.


### 3.3 Quality-of-life improvements

- **Streaming responses.** The structured Markdown output is long. Streaming it section by section would substantially improve perceived latency. Chainlit supports streaming patterns and should be used for this.
...
- **Containerisation and deployment.** The current app runs via Chainlit (`chainlit run chainlit_ui.py`) from a developer machine. Add a Dockerfile, a `docker-compose.yml` that pre-builds the Chroma stores on first run, and a deployment guide for a small VPS or managed container platform.
