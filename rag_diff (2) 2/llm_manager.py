...
class LLMManager:
    ...
    def _assert_torch_usable(self) -> None:
        version = tuple(int(x) for x in torch.__version__.split("+")[0].split(".")[:2])
        if version < (2, 4):
            raise RuntimeError(
                f"Installed torch version is {torch.__version__}, but this environment "
                "needs torch >= 2.4 for the current transformers stack. "
                "Upgrade torch or downgrade transformers."
            )

    def _assert_bitsandbytes_usable(self) -> None:
        if self.quantization_mode not in {"4bit", "8bit"}:
            return
        try:
            import bitsandbytes as bnb  # noqa: F401
        except Exception as e:
            raise RuntimeError(
                "bitsandbytes could not be imported, but quantization_mode requires it. "
                "Either fix bitsandbytes/CUDA or run with --quantization_mode none.\n"
                f"bitsandbytes import error: {e}"
            )
    ...
    def _load_tokenizer(self):
        try:
            ...
            print("[LLMManager] Loaded slow tokenizer successfully")
            return tok
        except Exception as e_slow:
            print(f"[LLMManager] Slow tokenizer failed: {e_slow}")
        ...
        try:
            ...
            print("[LLMManager] Loaded fast tokenizer successfully")
            return tok
        except Exception as e_fast:
            raise RuntimeError(
                "Both slow and fast tokenizer loading failed.\n"
                f"Slow tokenizer error: {e_slow}\n"
                f"Fast tokenizer error: {e_fast}"
            )

    def load_model(self) -> None:
        print(f"\n[LLMManager] Loading model: {self.model_name}")
        print(f"[LLMManager] torch.__version__={torch.__version__}")
        print(f"[LLMManager] CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', '')!r}")
        print(f"[LLMManager] torch.cuda.device_count()={self._visible_gpu_count()}")

        self._assert_torch_usable()
        self._assert_bitsandbytes_usable()
    ...
    def invoke_timed(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            prompt_tokens = len(self.tokenizer(formatted_prompt, add_special_tokens=False)["input_ids"])
        except Exception:
            prompt_tokens = 0
        ...
        try:
            output_tokens = len(self.tokenizer(out, add_special_tokens=False)["input_ids"])
        except Exception:
            output_tokens = 0
