...
class ChromaBalancedRAGRetriever:
    ...
    # ── Adaptive retrieval (RARC) ─────────────────────────────────────────────

    @staticmethod
    def _query_coverage(results: List[Dict[str, Any]], query: str) -> float:
        """Fraction of 4+ char query tokens that appear in the top-4 results."""
        import re as _re
        tokens = {t for t in _re.findall(r"[A-Za-z0-9]+", query.lower()) if len(t) >= 4}
        if not tokens:
            return 1.0
        joined = " ".join((r.get("content") or "") for r in results[:4]).lower()
        return sum(1 for t in tokens if t in joined) / len(tokens)

    def adaptive_retrieve(
        self,
        query: str,
        policy,           # rag_core.RetrievalPolicy
        score_threshold: float = 0.0,
    ) -> List[Dict[str, Any]]:
        """
        Retrieve using a per-query RetrievalPolicy (RARC).

        Temporarily overrides pool_size, neighbor_window, and max_per_source
        for the duration of the call, then restores them so concurrent use
        of the same retriever instance is safe.

        For policies with expand_if_low_coverage=True (AMBIGUOUS queries),
        a second pass with expansion_k is triggered if the initial retrieval
        covers fewer than coverage_threshold of the query tokens.
        """
        orig_pool     = self.pool_size
        orig_max_per  = self.max_per_source
        orig_neighbor = self.neighbor_window

        self.pool_size     = policy.pool_size
        self.max_per_source = policy.max_per_source
        self.neighbor_window = policy.neighbor_window

        try:
            results = self.retrieve(query, top_k=policy.k, score_threshold=score_threshold)

            if policy.expand_if_low_coverage and results and policy.expansion_k > policy.k:
                coverage = self._query_coverage(results, query)
                if coverage < policy.coverage_threshold:
                    print(
                        f"[AdaptiveRetrieve] Low coverage ({coverage:.2f} < "
                        f"{policy.coverage_threshold}) — expanding k: "
                        f"{policy.k} → {policy.expansion_k}"
                    )
                    self.pool_size = max(self.pool_size, policy.expansion_k * 2)
                    results = self.retrieve(
                        query, top_k=policy.expansion_k, score_threshold=score_threshold
                    )
        finally:
            self.pool_size      = orig_pool
            self.max_per_source  = orig_max_per
            self.neighbor_window = orig_neighbor
