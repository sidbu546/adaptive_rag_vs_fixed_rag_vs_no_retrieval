...
# 8-bit quantization for Qwen2.5-32B-Instruct on RTX A6000
# ============================================================
...
# 4) 8-bit is slower than 4-bit but typically more accurate.
# ============================================================
import os
...
OUTPUT_CSV_PATH = str(OUTPUTS_DIR / "fixed_rag_results_qwen32b_8bit_big.csv")
...
# Memory caps for RTX A6000 48GB class GPU.
# 8-bit Qwen2.5-32B needs ~33-35GB for weights alone, plus KV cache + activations.
# Start at 44GiB; drop to 42 or 40 GiB if OOM (will trigger CPU offload, slower).
GPU_MAX_MEMORY = "44GiB"
...
# LLM-judge config (same loaded model is reused as judge — see Section 11)
MAX_CLAIMS_PER_QUERY = 12     # cap on sentences judged per answer
JUDGE_MAX_NEW_TOKENS = 256    # judge outputs are short JSON
JUDGE_TEMPERATURE = 0.0
JUDGE_DO_SAMPLE = False

# Need json/re for parsing judge outputs
import json as _json_mod  # alias to avoid clobbering anything named `json` later
...
def load_qwen_model(local_model_path: str):
    ...
    print_header("Loading Qwen model safely (8-bit)")
...
# =========================
# 11) LLM-JUDGE EVALUATION
# =========================
# The same loaded model is reused as the judge (no second model loaded).
# Judge calls run AFTER generate_answer() returns, so they do NOT inflate
# llm_latency_s / gpu_throughput_toks_per_s for the answer-generation step.
#
# Hallucination rate and groundedness come from per-claim labeling:
#   supported          -> 1.0
#   partially_supported-> 0.5
#   unsupported        -> 0.0
# Hallucination rate = unsupported_count / claim_count
# Groundedness       = mean of per-claim scores
#
# Answer relevance and context relevance (1-5) come from structured 1-5 scores
# in the same judge output schema.
...
def safe_json_loads(s: str) -> Dict[str, Any]:
    """Parse JSON from a model response. Falls back to extracting the first
    {...} block if the model added preamble or markdown fences."""
    s = (s or "").strip()
    try:
...
def split_sentences(text: str) -> List[str]:
    """Split an answer into atomic claims. Handles:
      - Bullet/numbered lists (each bullet is a claim)
      - Newline-separated lines
      - Sentences ending in .!? (any continuation, not just uppercase)
      - Filters fragments shorter than MIN_CLAIM_CHARS to drop list headers like 'The components are:'
    """
    text = (text or "").strip()
    if not text:
        return []

    MIN_CLAIM_CHARS = 15  # drop trivially-short fragments

    # Step 1: split on hard line breaks first — bullets, numbered lists, newlines
    # are all structural separators in model output.
    ...
    for line in line_parts:
        ...
        # Step 2: within each line, split on sentence-final punctuation followed
        # by whitespace. Don't require an uppercase next char — models often
        # continue with lowercase, parens, or quotes.
    ...
    # If aggressive splitting yielded nothing (e.g., a single 12-char answer),
    # fall back to the whole text as one claim.
...
def judge_call_llm(model, tokenizer, prompt: str) -> str:
    """Run the loaded model as a judge with deterministic, short-output settings.
    Returns the generated text (no input echo). Not timed against the answer
    generation budget — call OUTSIDE the answer-timing block."""
    messages = [
        {"role": "system", "content": "You are a careful evaluator. Return ONLY valid JSON, no preamble, no markdown."},
...
def judge_claims(model, tokenizer, context: str, answer: str,
                 max_claims: int = MAX_CLAIMS_PER_QUERY) -> Dict[str, Any]:
    """Per-claim grounding judgment. Returns:
        {
          "claims": [{claim, label, evidence}, ...],
          "hallucination_rate": float,   # unsupported_count / claim_count
          "groundedness_score": float,   # mean of per-claim scores
...
def judge_relevance(model, tokenizer, question: str, answer: str, context: str) -> Dict[str, Any]:
    """Structured 1-5 relevance scores from the judge."""
...
# ----- Retained utilities (not replaced by the judge) -------------------
def estimate_confidence(top_score: float, avg_score: float, groundedness: float) -> float:
    # Combine retrieval strength + groundedness (groundedness now from LLM judge)
...
def main():
    ...
    for idx, query in enumerate(PROMPTS, start=1):
        ...
        # Build the joined context once for the judge (preserves passage boundaries
        # the same way build_rag_messages does, so the judge sees what the model saw).
        ...
        # ---- LLM-judge evaluation (runs AFTER answer generation, outside timing) ----
        ...
        # Confidence and coverage are independent of the judge step.
